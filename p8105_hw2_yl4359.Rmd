...
There are `r nrow(wheel_data)` observations in _wheel_data_, 
`r nrow(precipitation_data)` observations in _precipitation_data_.
The key variables in _wheel_data_ are `dumpster`, `month`, `year` and `weight_tons`.
The total precipitation in 2018 is `r sum(pull(precipitation_2018_data, total))`. 
The median number of sports balls in a dumpster in 2017 is `r median(pull(filter(wheel_data, year == "2017"), sports_balls))`.
...
The dataset _pols_month_data_ contains `r nrow(pols_month_data)` rows and `r ncol(pols_month_data)` columns.
The dataset _snp_data_ contains `r nrow(snp_data)` rows and `r ncol(snp_data)` columns.
The dataset _unemployment_data_ contains `r nrow(unemployment_data)` and `r ncol(unemployment_data)` columns.
The resulting dataset _pols_snp_unemploy_data_ contains `r nrow(pols_snp_unemploy_data)` rows and `r ncol(pols_snp_unemploy_data)` columns. 
The year in resulting dataset is from `r min(pull(pols_snp_unemploy_data, year))` to `r max(pull(pols_snp_unemploy_data, year))`.
The key variables in the resulting dataset includes `year`, `month`, `gov_gop`, `sen_gop`, `rep_gop`, `gov_dem`, `sen_dem`, `rep_dem`, `president`, `close` and `precentage`.
...
**Produce a well_structured, reader_friendly table showing the rank in popularity of the name "Olivia" as a female baby name over time**
